...
def main():
    ...
    def render_outputs():
        ...
        if found:
            ...
            st.info("Aucune figure trouvée. Assurez-vous d'avoir exécuté le notebook complet.")
        ...
        try:
            if os.path.exists(csv_path):
                ...
                st.info("Le fichier CSV de comparaison n'a pas été trouvé.")
        except Exception as e:
        ...
        # Metrics (optional - can be removed if redundant with text outputs)
        metrics_path = os.path.join(nb_outputs_dir, 'notebook_metrics.json')
        if os.path.exists(metrics_path):
            st.markdown('---')
            with st.expander('📊 Métriques Extraites (JSON détaillé)'):
                try:
                    import json
        ...
        # Full text dump (optional - for advanced users)
        text_out_path = os.path.join(nb_outputs_dir, 'notebook_text_outputs.txt')
        if os.path.exists(text_out_path):
            st.markdown('---')
            with st.expander('📝 Texte Complet des Sorties (Brut)'):
                try:
                    with open(text_out_path, 'r', encoding='utf-8') as tf:
                        txt = tf.read()
                    st.text_area('Outputs Notebook', value=txt, height=400, key='full_text_output')
                    try:
                        with open(text_out_path, 'rb') as bf:
                            b = bf.read()
                        st.download_button('📥 Télécharger le fichier texte', data=b, file_name='notebook_text_outputs.txt')
                    except Exception:
                        pass
                except Exception as e:
